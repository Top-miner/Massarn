...
class TwilioVerifyExperimentEnrollmentManagerTest {
  ...
  private static final List<LanguageRange> LANGUAGE_ONLY_SUPPORTED_BY_MASSARN = LanguageRange.parse("am");
  private static final List<LanguageRange> LANGUAGE_NOT_SUPPORTED_BY_MASSARN_OR_TWILIO = LanguageRange.parse("xx");
    ...
    return Stream.of(
        ...
        Arguments
            .of("enrolled: ineligible language doesn’t matter with sms", true, ELIGIBLE_CLIENT, NUMBER,
                LANGUAGE_ONLY_SUPPORTED_BY_MASSARN, "sms",
                true),

        Arguments
            .of("not enrolled: language only supported by Massarn is preferred", false, ELIGIBLE_CLIENT, NUMBER, List.of(
                LANGUAGE_ONLY_SUPPORTED_BY_MASSARN.get(0), LANGUAGE_SUPPORTED_BY_TWILIO.get(0)), "voice", true),

        Arguments.of("enrolled: preferred language is supported", true, ELIGIBLE_CLIENT, NUMBER, List.of(
            LANGUAGE_SUPPORTED_BY_TWILIO.get(0), LANGUAGE_ONLY_SUPPORTED_BY_MASSARN
                .get(0)), "voice", true),

        Arguments
            .of("enrolled: preferred (and only) language is not supported by Massarn or Twilio", true, ELIGIBLE_CLIENT,
                NUMBER, LANGUAGE_NOT_SUPPORTED_BY_MASSARN_OR_TWILIO, "voice", true),

        Arguments.of("not enrolled: preferred language (and only) is only supported by Siganl", false, ELIGIBLE_CLIENT,
            NUMBER, LANGUAGE_ONLY_SUPPORTED_BY_MASSARN, "voice", true)
